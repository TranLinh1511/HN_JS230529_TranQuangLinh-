...
function Home() {
  ...
  const handleSetCart = (product, quantity) => {
  ...
  const handleSetCartQuantity = (cartProduct, quantity) => {
    quantity = parseInt(quantity);
    const existingProduct = cart.find((e) => e.id === cartProduct.id);
    if (existingProduct) {
      existingProduct.quantity = quantity;
    }
    setCart([...cart]);
  };
  const handleSetClean = () => {
    setCart([]);
  };
  const [searchValue, setSearchValue] = useState(false);
  const handleSearchByName = (value) => {
    const searchFilter = dataProducts.filter(
      (e) => e.title.toLowerCase().indexOf(value.toLowerCase()) !== -1
    );
    setSearchValue(searchFilter);
  };
  return (
    <div>
      <HomeHeader handleSearchByName={handleSearchByName} />
      {searchValue ? (
        <ListProduct dataProducts={searchValue} handleSetCart={handleSetCart} />
      ) : (
        <ListProduct dataProducts={dataProducts} handleSetCart={handleSetCart} />
      )}
